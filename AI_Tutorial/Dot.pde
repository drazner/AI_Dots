class Dot {
  ...
  void update() {
    if (!dead && !reachedGoal) {
      move();
      if (pos.x < border|| pos.y < border || pos.x > width - border || pos.y > height - border) { //if near the edges of the window then kill it 
        dead = true;
      } else if (dist(pos.x, pos.y, goal.x, goal.y) < size+1) {//if reached goal
